...
namespace Whisper
{
	...
	using pfnNewSegment = HRESULT( __cdecl* )( iContext* ctx, uint32_t n_new, void* user_data );

	// Return S_OK to proceed, or S_FALSE to stop the process and return S_OK from runFull / runStreamed method
	using pfnEncoderBegin = HRESULT( __cdecl* )( iContext* ctx, void* user_data );
	...
	using pfnReportProgress = HRESULT( __stdcall* )( double val, iContext* ctx, void* pv );
